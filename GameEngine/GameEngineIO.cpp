...
void GameEngineIO::loadGameInfo(std::string fileName) {
    ...
    // want to have a check for the old game format, so we know what way to load in the game.
    oldGameFormat = false;

    // check for if it should try the old game format.
    if ((gameInfo[0] != "AI" && !gameEngine->inputIsInt(gameInfo[0])) || !gameEngine->inputIsInt(gameInfo[1])) {
        // new save format has first two lines as numbers, or first as "AI".
        oldGameFormat = true;
    }
}
...
void GameEngineIO::loadReplay(std::string fileName) {
    ...
    if (!oldGameFormat) {
        ...
        if (!readError) {
            ...
            int sizeGameInfo = gameInfo.size();
            
            if (gameInfoIndex >= sizeGameInfo) {
                // this runs when there's nothing after the seed
                readError = true;
            } else if (gameInfo[gameInfoIndex] == ""){
                readError = true;
            }
    ...
    if(readError){
        throw "Error loading as replay. Replay mode not available on old save game format.";
    }
...
void GameEngineIO::loadPlayers(bool replayMode) {
    ...
    if (!readError) {
        gameEngine->setUp(names, this->numPlayers, this->numNormalFactories, this->numCentral, AIMode, true, oldGameFormat);
...
void GameEngineIO::saveGame(std::string fileName, bool oldSave) {
    std::ofstream outFile;
    outFile.open(fileName);

    // the old save format doesn't store the number of players, or central factories.
    if (!oldSave) {
    ...
    if(outFile.good()){
        ...
        // save the game log only if it's not the old game format.
...
bool GameEngineIO::getReadError() {
    return readError;
}
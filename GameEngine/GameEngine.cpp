...
GameEngine::GameEngine(const int seed, bool AItesting) {
    commonGameEngine();
    this->seed = seed;
    this->testing = true;
...
void GameEngine::setPlayerStartingNextRound(){
    // check all player's broken tiles and the player with first player tile is who starts next.
    int maxBrokenTiles = 7;
    for (int player = 0; player < numPlayers; ++player) {
        std::shared_ptr<Tile>* brokenTiles = players[player]->getMosaicStorage()->getBrokenTiles()->getTiles();
        for (int tile = 0; tile < maxBrokenTiles; ++tile) {
            if (brokenTiles[tile] != nullptr && brokenTiles[tile]->getType() == Type::FIRST_PLAYER) {
                this->playerStartingNextRound = players[player]->getName();
            }
        }
    }
}
...
void GameEngine::newGame(const std::string player1Name, unsigned const int numNormalFactories,
    ...
    names[0] = player1Name;
    setUp(names, numPlayers, numNormalFactories, numCentralFactories, true, false, false);
}
...
void GameEngine::newGame(const std::string player1Name, const std::string player2Name, unsigned const int numNormalFactories,
    ...
    names[0] = player1Name;
    names[1] = player2Name;
    setUp(names, numPlayers, numNormalFactories, numCentralFactories, false, false, false);
}
...
void GameEngine::setUp(std::string* names, int numPlayers, int numNormalFactories, int numCentralFactories, bool AIMode, bool loading, bool oldGame) {
    ...
    for (int player = 0; player < numPlayers; ++player) {
        if (AIMode && player == 1) {
            ...
            // if AI testing is enabled then all players are AI
            if (AItesting) {
...
int GameEngine::playerTurn(std::string playerTurnCommand){
    ...
    if(commands[0] == "turn"){
        ...
        geIO->saveGame(commands[1], oldGameFormat);
...
void GameEngine::populateFactories(){
    ...
    for(int i = startIndex; i < numFactories; i++){
        ...
        for(int j = 0; j < 4; ++j){
            if (bag->getSize() > 0) {
                tileAddedThisRound = true;
                factory[i]->addTile(bag->getAndRemoveFirstTile());
            } else {
                refillBag();
                // don't add if the size is still 0 because it will segfault
                if (bag->getSize() != 0) {
                    factory[i]->addTile(bag->getAndRemoveFirstTile());
                }
            }
    ...
    // if not a single thing was added in the factories then there are no more tiles left in the bag at all, end the game.
    if (!tileAddedThisRound) {
        noMoreTiles = true;
    }
}
...
void GameEngine::gameplayLoop(bool& endOfCommands, bool& continueMenuLoop) {
    ...
    // since loading in complete games is possible, must ensure we don't do end of game calculations upon loading them.
    bool gameAlreadyCompleted = true;
    Input input;
    while(!endOfCommands && !std::cin.eof() && !winConditionMet() && continueAfterReplay && !noMoreTiles){
        gameAlreadyCompleted = false;
    ...
    if (winConditionMet() || noMoreTiles) {
        ...
        if (testing) {
            GameEngineIO* geIO = new GameEngineIO(this);
            geIO->saveGame("actualoutcome.save", oldGameFormat);
            delete geIO;
        }
        ...
        if (saveChoice == "YES") {
            ...
            GameEngineIO* geIO = new GameEngineIO(this);
            geIO->saveGame(fileName, oldGameFormat);
            delete geIO;
        }
...
void GameEngine::resetGame(){
    // new players are made with each new game instance, so we need to reset these so there's no memory leaks.
